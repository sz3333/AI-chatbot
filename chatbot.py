...
async def cmd_ban(msg: Message):
    ...
    if len(args) < 2 or not args[1].lstrip("-").isdigit():
        return await msg.answer(
            "Использование: /ban &lt;user_id&gt; [секунды]\n"
            "Без указания секунд — стандартный бан на 5 минут.",
            parse_mode=ParseMode.HTML,
        )
    uid = int(args[1])
    duration = int(args[2]) if len(args) >= 3 and args[2].isdigit() else SPAM_BAN_DURATION
    rate_limiter.manual_ban(uid, duration)
    log.info(f"[MANUAL BAN] uid={uid} на {duration}с от владельца")
    await msg.answer(
        f"🔨 Пользователь <code>{uid}</code> забанен на <b>{duration}</b> секунд.",
        parse_mode=ParseMode.HTML,
    )
...
async def cmd_unban(msg: Message):
    ...
    uid = int(args[1])
    rate_limiter.unban(uid)
    await msg.answer(f"✅ Пользователь <code>{uid}</code> разбанен.", parse_mode=ParseMode.HTML)
...
async def handle_text(msg: Message, bot: Bot):
    ...
    # ── Антиспам проверка (владельца не трогаем) ─────────────────────────────
    if uid != OWNER_ID:
        status, ban_left = rate_limiter.check(uid)
        if status == "banned":
            # Уже был забанен — тихо игнорируем
            return
        elif status == "ban":
            # Только что получил бан
            try:
                await msg.answer(
                    f"🚫 Ты отправляешь сообщения слишком быстро.\n"
                    f"Бан на <b>{ban_left}</b> секунд. Успокойся.",
                    parse_mode=ParseMode.HTML,
                )
            except Exception:
                pass
            log.warning(f"[ANTISPAM] Автобан uid={uid} chat={msg.chat.id}")
            return
        elif status == "warn":
            try:
                await msg.answer(
                    f"⚠️ Не спамь! Ещё {SPAM_BAN_AFTER - SPAM_MAX_MESSAGES} "
                    f"быстрых сообщений — и получишь бан.",
                )
            except Exception:
                pass
            return
        elif status == "spam":
            # Молча игнорируем после предупреждения
            return
    # ─────────────────────────────────────────────────────────────────────────
